...
type
  ...
  TLzToolbarButton = class(TGraphicControl)
  private
    FAllowAllUp: Boolean;
  ...
  {$IFDEF TB97D4}
  TLzToolbarButtonActionLink = class(TControlActionLink)
  protected
    FClient: TLzToolbarButton;
...
var
  ...
  ButtonMouseInControl: TLzToolbarButton = nil;
...
type
  TLzCustomToolWindowAccess = class(TLzCustomToolWindow);
...
{$IFDEF TB97D4}
procedure TLzToolbarButtonActionLink.AssignClient (AClient: TObject);
begin
  inherited AssignClient(AClient);
  FClient := AClient as TLzToolbarButton;
end;

function TLzToolbarButtonActionLink.IsCheckedLinked: Boolean;
begin
  Result := inherited IsCheckedLinked and
    (FClient.Down = (Action as TCustomAction).Checked);
end;

function TLzToolbarButtonActionLink.IsHelpContextLinked: Boolean;
begin
  Result := inherited IsHelpContextLinked and
    (FClient.HelpContext = (Action as TCustomAction).HelpContext);
end;

function TLzToolbarButtonActionLink.IsImageIndexLinked: Boolean;
begin
  Result := inherited IsImageIndexLinked and
    (FClient.ImageIndex = (Action as TCustomAction).ImageIndex);
end;

procedure TLzToolbarButtonActionLink.SetChecked (Value: Boolean);
begin
  if IsCheckedLinked then FClient.Down := Value;
end;

procedure TLzToolbarButtonActionLink.SetHelpContext (Value: THelpContext);
begin
  if IsHelpContextLinked then FClient.HelpContext := Value;
end;

procedure TLzToolbarButtonActionLink.SetImageIndex (Value: Integer);
begin
...
destructor TLzToolbarButton.Destroy;
begin
...
procedure TLzToolbarButton.Paint;
const
...
procedure TLzToolbarButton.RemoveButtonMouseTimer;
begin
...
procedure TLzToolbarButton.Loaded;
var
  State: TButtonStateX2;
begin
  inherited;
...
procedure TLzToolbarButton.Notification (AComponent: TComponent; Operation: TOperation);
begin
  inherited;
  if Operation = opRemove then begin
...
function TLzToolbarButton.PointInButton (X, Y: Integer): Boolean;
begin
  Result := (X >= 0) and
...
procedure TLzToolbarButton.MouseDown (Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TLzToolbarButton.MouseMove (Shift: TShiftState; X, Y: Integer);
var
  P: TPoint;
...
procedure TLzToolbarButton.RepeatTimerHandler (Sender: TObject);
var
  P: TPoint;
begin
...
procedure TLzToolbarButton.WMCancelMode (var Message: TWMCancelMode);
begin
  FRepeatTimer.Free;
  FRepeatTimer := nil;
  if FMouseIsDown then begin
...
procedure TLzToolbarButton.MouseUp (Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  FRepeatTimer.Free;
  FRepeatTimer := nil;
...
procedure TLzToolbarButton.Click;
{$IFNDEF TB97D4}
const
...
{$IFDEF USEBUTTONGLYPH}
function TLzToolbarButton.GetPalette: HPALETTE;
begin
  Result := Glyph.Palette;
end;

function TLzToolbarButton.GetGlyph: TBitmap;
begin
  Result := TButtonGlyph(FGlyph).Glyph;
end;

procedure TLzToolbarButton.SetGlyph (Value: TBitmap);
begin
  TButtonGlyph(FGlyph).Glyph := Value;
  Redraw (True);
end;

function TLzToolbarButton.GetGlyphMask: TBitmap;
begin
  Result := TButtonGlyph(FGlyph).GlyphMask;
end;

procedure TLzToolbarButton.SetGlyphMask (Value: TBitmap);
begin
...
procedure TLzToolbarButton.SetHighlightWhenDown (Value: Boolean);
begin
...
function TLzToolbarButton.GetImageIndex: TImageIndex;
begin
  {$IFDEF USEBUTTONGLYPH}
...
procedure TLzToolbarButton.SetImageIndex (Value: TImageIndex);
begin
  {$IFDEF USEBUTTONGLYPH}
...
function TLzToolbarButton.GetImages: TCustomImageList;
begin
  {$IFDEF USEBUTTONGLYPH}
...
procedure TLzToolbarButton.SetImages (Value: TCustomImageList);
begin
  {$IFDEF USEBUTTONGLYPH}
...
{$IFDEF USEBUTTONGLYPH}
function TLzToolbarButton.GetNumGlyphs: TNumGlyphsX2;
begin
  Result := TButtonGlyph(FGlyph).NumGlyphs;
end;

procedure TLzToolbarButton.SetNumGlyphs (Value: TNumGlyphsX2);
begin
...
procedure TLzToolbarButton.GlyphChanged(Sender: TObject);
begin
  Redraw (True);
end;

procedure TLzToolbarButton.UpdateExclusive;
var
  I: Integer;
  Ctl: TControl;
begin
  if (FGroupIndex <> 0) and (Parent <> nil) then
    with Parent do
      for I := 0 to ControlCount-1 do begin
        Ctl := Controls[I];
        if (Ctl <> Self) and (Ctl is TLzToolbarButton) then
          with TLzToolbarButton(Ctl) do
...
procedure TLzToolbarButton.SetDown (Value: Boolean);
begin
...
procedure TLzToolbarButton.SetFlat (Value: Boolean);
begin
...
procedure TLzToolbarButton.SetGroupIndex (Value: Integer);
begin
...
procedure TLzToolbarButton.SetLayout (Value: TButtonLayout);
begin
...
procedure TLzToolbarButton.SetMargin (Value: Integer);
begin
...
procedure TLzToolbarButton.SetNoBorder (Value: Boolean);
begin
...
procedure TLzToolbarButton.SetOldDisabledStyle (Value: Boolean);
begin
  {$IFDEF USEBUTTONGLYPH}
...
procedure TLzToolbarButton.SetOpaque (Value: Boolean);
begin
...
procedure TLzToolbarButton.Redraw (const Erase: Boolean);
var
  AddedOpaque: Boolean;
begin
...
procedure TLzToolbarButton.SetSpacing (Value: Integer);
begin
...
procedure TLzToolbarButton.SetAllowAllUp (Value: Boolean);
begin
...
procedure TLzToolbarButton.SetDropdownMenu (Value: TPopupMenu);
var
...
procedure TLzToolbarButton.SetWordWrap (Value: Boolean);
begin
...
procedure TLzToolbarButton.SetAlignment (Value: TAlignment);
begin
...
procedure TLzToolbarButton.SetDropdownAlways (Value: Boolean);
var
  NewUsesDropdown: Boolean;
begin
...
procedure TLzToolbarButton.SetDropdownArrow (Value: Boolean);
begin
...
procedure TLzToolbarButton.SetDropdownArrowWidth (Value: Integer);
var
  Diff: Integer;
begin
...
procedure TLzToolbarButton.SetDropdownCombo (Value: Boolean);
var
  W: Integer;
begin
...
procedure TLzToolbarButton.SetDisplayMode (Value: TButtonDisplayMode);
begin
...
{$IFDEF USEBUTTONGLYPH}
function TLzToolbarButton.GetCallDormant: Boolean;
begin
  Result := TButtonGlyph(FGlyph).FCallDormant;
end;

procedure TLzToolbarButton.SetCallDormant (Value: Boolean);
begin
...
{$IFDEF TB97D4}
function TLzToolbarButton.IsCheckedStored: Boolean;
begin
  Result := (ActionLink = nil) or not TLzToolbarButtonActionLink(ActionLink).IsCheckedLinked;
end;

function TLzToolbarButton.IsHelpContextStored: Boolean;
begin
  Result := (ActionLink = nil) or not TLzToolbarButtonActionLink(ActionLink).IsHelpContextLinked;
end;

function TLzToolbarButton.IsImageIndexStored: Boolean;
begin
  Result := (ActionLink = nil) or not TLzToolbarButtonActionLink(ActionLink).IsImageIndexLinked;
end;

procedure TLzToolbarButton.ActionChange (Sender: TObject; CheckDefaults: Boolean);
begin
  inherited;
...
function TLzToolbarButton.GetActionLinkClass: TControlActionLinkClass;
begin
  Result := TLzToolbarButtonActionLink;
end;

procedure TLzToolbarButton.AssignTo (Dest: TPersistent);
begin
  inherited;
...
procedure TLzToolbarButton.WMLButtonDblClk (var Message: TWMLButtonDblClk);
begin
  inherited;
  if FDown then DblClick;
end;

procedure TLzToolbarButton.CMEnabledChanged (var Message: TMessage);
begin
  if not Enabled then begin
    FState := bsDisabled;
...
procedure TLzToolbarButton.CMDialogChar (var Message: TCMDialogChar);
begin
  with Message do
...
procedure TLzToolbarButton.CMDialogKey (var Message: TCMDialogKey);
begin
  with Message do
...
procedure TLzToolbarButton.CMFontChanged (var Message: TMessage);
begin
  Redraw (True);
end;

procedure TLzToolbarButton.CMTextChanged (var Message: TMessage);
begin
  Redraw (True);
end;

procedure TLzToolbarButton.CMSysColorChange (var Message: TMessage);
begin
  inherited;
...
procedure TLzToolbarButton.MouseEntered;
begin
...
procedure TLzToolbarButton.MouseLeft;
var
...
procedure TLzToolbarButton.ButtonMouseTimerHandler (Sender: TObject);
var
  P: TPoint;
begin
...
begin
  ...
  if EditX2Count = 0 then
    RegisterX2ControlClass (TLzEdit);
  Inc (EditX2Count);
end;
...
begin
  Dec (EditX2Count);
  if EditX2Count = 0 then
    UnregisterX2ControlClass (TLzEdit);
  inherited;
end;

procedure TLzEdit.CMMouseEnter (var Message: TMessage);
begin
  inherited;
  MouseInControl := True;
...
procedure TLzEdit.CMMouseLeave (var Message: TMessage);
begin
  inherited;
...
procedure TLzEdit.NewAdjustHeight;
var
  DC: HDC;
  SaveFont: HFONT;
  Metrics: TTextMetric;
...
procedure TLzEdit.CMEnabledChanged (var Message: TMessage);
const
...
procedure TLzEdit.CMFontChanged (var Message: TMessage);
begin
  inherited;
...
procedure TLzEdit.WMSetFocus (var Message: TWMSetFocus);
begin
  inherited;
  if not(csDesigning in ComponentState) then
    DrawNCArea (False, 0, 0);
...
procedure TLzEdit.WMKillFocus (var Message: TWMKillFocus);
begin
  inherited;
  if not(csDesigning in ComponentState) then
    DrawNCArea (False, 0, 0);
...
procedure TLzEdit.WMNCCalcSize (var Message: TWMNCCalcSize);
begin
  InflateRect (Message.CalcSize_Params^.rgrc[0], -3, -3);
end;

procedure TLzEdit.WMNCPaint (var Message: TMessage);
begin
  DrawNCArea (False, 0, HRGN(Message.WParam));
end;

procedure TLzEdit.DrawNCArea (const DrawToDC: Boolean; const ADC: HDC;
  const Clip: HRGN);
var
  DC: HDC;
  R: TRect;
...
begin
  TLzEdit(AppData).DrawNCArea (True, DC, 0);
end;
...
function TLzToolbarButton.GetAvailableImageList: TCustomImageList;
...
begin
  ...
  begin
    if Parent is TLzCustomToolWindow then
    begin
      Result := TLzCustomToolWindowAccess(Parent).Images;
    end;  
  end;  
end;
