...
type
  ...
  private
    ...
    FDefaultDock: TDockX2; { True if Close button is currently depressed }
        ...
        with TCustomToolWindowX2(DockList[I]) do
          if (FDockRow = R) and FullSize then
            for J := 0 to DockList.Count-1 do
              if (J <> I) and (TCustomToolWindowX2(DockList[J]).FDockRow = R) then begin
...
begin
  ...
  while R <= HighestRow do begin
    RowIsEmpty := True;
    for I := 0 to DockList.Count-1 do
      if TCustomToolWindowX2(DockList[I]).FDockRow = R then begin
        RowIsEmpty := False;
        Break;
      end;
