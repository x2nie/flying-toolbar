...
type
  ...
  TButtonStateX2 = (bsUp, bsDisabled, bsDown, bsExclusive, bsMouseIn);
  TNumGlyphsX2 = 1..5;
  TButtonDropdownEvent = procedure(Sender: TObject;
    var ShowMenu, RemoveClicks: Boolean) of object;

  TToolbarButtonX2 = class(TGraphicControl)
  private
    FAllowAllUp: Boolean;
    ...
    FFlat: Boolean;
    {$IFDEF USEBUTTONGLYPH}
    FGlyph: Pointer;
    {$ELSE}
    FImageList: TCustomImageList;
    FImageIndex : Integer;// TImageIndex;
    {$ENDIF}
    FGroupIndex: Integer;
    ...
    function GetNumGlyphs: TNumGlyphsX2;
    procedure SetNumGlyphs (Value: TNumGlyphsX2);
  ...
  protected
    ...
    {$ENDIF}
    function GetAvailableImageList : TCustomImageList;
  public
    property Canvas;
  ...
  published
    ...
    property NumGlyphs: TNumGlyphsX2 read GetNumGlyphs write SetNumGlyphs default 1;
    {$ENDIF}
    ...
    property ImageIndex: TImageIndex read GetImageIndex write SetImageIndex {$IFDEF TB97D4} stored IsImageIndexStored {$ENDIF} default -1;
  ...
  {$IFDEF TB97D4}
  TToolbarButtonX2ActionLink = class(TControlActionLink)
  protected
    FClient: TToolbarButtonX2;
  ...
  TToolbarButtonX2ActionLinkClass = class of TToolbarButtonX2ActionLink;
  {$ENDIF}

  { TEdit97 }

  TEditX2 = class(TCustomEdit)
  private
...
var
  ...
  ButtonMouseInControl: TToolbarButtonX2 = nil;

function ControlIsX2Control (AControl: TControl): Boolean;
procedure RegisterX2ControlClass (AClass: TClass);
procedure UnregisterX2ControlClass (AClass: TClass);

implementation

uses
  SysUtils, {$IFDEF FPC}LCLStrConsts, {$ELSE} Consts, {$ENDIF}    CommCtrl,
  TBx2,
  TBx2_Common;

type
  TCustomToolWindowX2Access = class(TCustomToolWindowX2);
    
var
  { See TToolbarButton97.ButtonMouseTimerHandler for info on this }
  ButtonMouseTimer: TTimer = nil;

  ControlX2List: TList = nil;

  EditX2Count: Integer = 0;
...
const
  ...
  //http://orion.lcg.ufrj.br/RPMS/myrpms/lazarus/lazarus-components/lazreport/source/lr_fpc.pas
  var
    LP: ^TLogPalette;
    NumEntries: integer;
  begin
...
begin
  Result := False;
  if Assigned(AControl) and Assigned(ControlX2List) then
    for I := 0 to ControlX2List.Count-1 do
      if AControl is TClass(ControlX2List[I]) then begin
        Result := True;
        Break;
      end;
end;
...
procedure RegisterX2ControlClass (AClass: TClass);
begin
  if ControlX2List = nil then ControlX2List := TList.Create;
  ControlX2List.Add (AClass);
end;

procedure UnregisterX2ControlClass (AClass: TClass);
begin
  if Assigned(ControlX2List) then begin
    ControlX2List.Remove (AClass);
    if ControlX2List.Count = 0 then begin
      ControlX2List.Free;
      ControlX2List := nil;
    end;
  end;
...
{$IFDEF TB97D4}
procedure TToolbarButtonX2ActionLink.AssignClient (AClient: TObject);
begin
  inherited AssignClient(AClient);
  FClient := AClient as TToolbarButtonX2;
end;

function TToolbarButtonX2ActionLink.IsCheckedLinked: Boolean;
begin
  Result := inherited IsCheckedLinked and
    (FClient.Down = (Action as TCustomAction).Checked);
end;

function TToolbarButtonX2ActionLink.IsHelpContextLinked: Boolean;
begin
  Result := inherited IsHelpContextLinked and
    (FClient.HelpContext = (Action as TCustomAction).HelpContext);
end;

function TToolbarButtonX2ActionLink.IsImageIndexLinked: Boolean;
begin
  Result := inherited IsImageIndexLinked and
    (FClient.ImageIndex = (Action as TCustomAction).ImageIndex);
end;

procedure TToolbarButtonX2ActionLink.SetChecked (Value: Boolean);
begin
  if IsCheckedLinked then FClient.Down := Value;
end;

procedure TToolbarButtonX2ActionLink.SetHelpContext (Value: THelpContext);
begin
  if IsHelpContextLinked then FClient.HelpContext := Value;
end;

procedure TToolbarButtonX2ActionLink.SetImageIndex (Value: Integer);
begin
...
type
  ...
  private
    ...
    FIndexs: array[Boolean, TButtonStateX2] of Integer;
    FTransparentColor: TColor;
    FNumGlyphs: TNumGlyphsX2;
    ...
    function CreateButtonGlyph (State: TButtonStateX2): TBoolInt;
    procedure DrawButtonGlyph (Canvas: TCanvas; const GlyphPos: TPoint;
      State: TButtonStateX2);
    procedure DrawButtonText (Canvas: TCanvas;
      const Caption: string; TextBounds: TRect;
      WordWrap: Boolean; Alignment: TAlignment; State: TButtonStateX2);
  ...
  public
    ...
    function Draw (Canvas: TCanvas; const Client: TRect; const Offset: TPoint;
      ...
      DropArrow: Boolean; DropArrowWidth: Integer; State: TButtonStateX2): TRect;
    property Glyph: TBitmap read FOriginal write SetGlyph;
    property GlyphMask: TBitmap read FOriginalMask write SetGlyphMask;
    property NumGlyphs: TNumGlyphsX2 read FNumGlyphs write SetNumGlyphs;
...
begin
  if (FOriginal.Width <> 0) and (FOriginal.Height <> 0) and
    ...
    if Glyphs > High(TNumGlyphsX2) then Glyphs := 1;
  end
  else
    Glyphs := 1;
  SetNumGlyphs (Glyphs);
...
procedure TButtonGlyph.SetNumGlyphs (Value: TNumGlyphsX2);
begin
  Invalidate;
  if (FImageList <> nil) or (Value < Low(TNumGlyphsX2)) or
     (Value > High(TNumGlyphsX2)) then
    FNumGlyphs := 1
  else
    FNumGlyphs := Value;
  GlyphChanged (nil);
...
function TButtonGlyph.CreateButtonGlyph (State: TButtonStateX2): TBoolInt;
const
...
procedure TButtonGlyph.DrawButtonDropArrow (Canvas: TCanvas;
  const X, Y, AWidth: Integer; State: TButtonStateX2);
var
  X2: Integer;
begin
  with Canvas do begin
...
constructor TToolbarButtonX2.Create (AOwner: TComponent);
begin
  inherited;
...
destructor TToolbarButtonX2.Destroy;
begin
...
procedure TToolbarButtonX2.Paint;
const
...
begin
  ...
  try
    ...
    {$ELSE}
      ...
      begin
        ImgList := GetAvailableImageList();
        if Assigned(ImgList) then
          ImgList.Draw( DrawCanvas, PaintRect.Left, PaintRect.Top, self.ImageIndex, self.Enabled)
      end;
    {$ENDIF}
...
procedure TToolbarButtonX2.RemoveButtonMouseTimer;
begin
...
procedure TToolbarButtonX2.Loaded;
var
  State: TButtonStateX2;
begin
  inherited;
...
procedure TToolbarButtonX2.Notification (AComponent: TComponent; Operation: TOperation);
begin
  inherited;
  if Operation = opRemove then begin
    if AComponent = DropdownMenu then DropdownMenu := nil;
    if {$IFDEF USEBUTTONGLYPH} Assigned(FGlyph) and {$ENDIF} (AComponent = Images) then Images := nil;
  end;
end;

function TToolbarButtonX2.PointInButton (X, Y: Integer): Boolean;
begin
  Result := (X >= 0) and
...
procedure TToolbarButtonX2.MouseDown (Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TToolbarButtonX2.MouseMove (Shift: TShiftState; X, Y: Integer);
var
  P: TPoint;
  NewState: TButtonStateX2;
  PtInButton: Boolean;
begin
  inherited;
...
procedure TToolbarButtonX2.RepeatTimerHandler (Sender: TObject);
var
  P: TPoint;
begin
...
procedure TToolbarButtonX2.WMCancelMode (var Message: TWMCancelMode);
begin
  FRepeatTimer.Free;
  FRepeatTimer := nil;
  if FMouseIsDown then begin
...
procedure TToolbarButtonX2.MouseUp (Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  FRepeatTimer.Free;
  FRepeatTimer := nil;
...
procedure TToolbarButtonX2.Click;
{$IFNDEF TB97D4}
const
...
{$IFDEF USEBUTTONGLYPH}
function TToolbarButtonX2.GetPalette: HPALETTE;
begin
  Result := Glyph.Palette;
end;

function TToolbarButtonX2.GetGlyph: TBitmap;
begin
  Result := TButtonGlyph(FGlyph).Glyph;
end;

procedure TToolbarButtonX2.SetGlyph (Value: TBitmap);
begin
  TButtonGlyph(FGlyph).Glyph := Value;
  Redraw (True);
end;

function TToolbarButtonX2.GetGlyphMask: TBitmap;
begin
  Result := TButtonGlyph(FGlyph).GlyphMask;
end;

procedure TToolbarButtonX2.SetGlyphMask (Value: TBitmap);
begin
...
procedure TToolbarButtonX2.SetHighlightWhenDown (Value: Boolean);
begin
...
function TToolbarButtonX2.GetImageIndex: TImageIndex;
begin
  {$IFDEF USEBUTTONGLYPH}
...
procedure TToolbarButtonX2.SetImageIndex (Value: TImageIndex);
begin
  {$IFDEF USEBUTTONGLYPH}
  ...
  if FImageIndex <> Value then begin
    FImageIndex := Value;
    Self.GlyphChanged(Self);
  end;
  {$ENDIF}

end;

function TToolbarButtonX2.GetImages: TCustomImageList;
begin
  {$IFDEF USEBUTTONGLYPH}
...
procedure TToolbarButtonX2.SetImages (Value: TCustomImageList);
begin
  {$IFDEF USEBUTTONGLYPH}
...
{$IFDEF USEBUTTONGLYPH}
function TToolbarButtonX2.GetNumGlyphs: TNumGlyphsX2;
begin
  Result := TButtonGlyph(FGlyph).NumGlyphs;
end;

procedure TToolbarButtonX2.SetNumGlyphs (Value: TNumGlyphsX2);
begin
  if Value < Low(TNumGlyphsX2) then
    Value := Low(TNumGlyphsX2)
  else
  if Value > High(TNumGlyphsX2) then
    Value := High(TNumGlyphsX2);
...
procedure TToolbarButtonX2.GlyphChanged(Sender: TObject);
begin
  Redraw (True);
end;

procedure TToolbarButtonX2.UpdateExclusive;
var
  I: Integer;
  Ctl: TControl;
begin
  if (FGroupIndex <> 0) and (Parent <> nil) then
    with Parent do
      for I := 0 to ControlCount-1 do begin
        Ctl := Controls[I];
        if (Ctl <> Self) and (Ctl is TToolbarButtonX2) then
          with TToolbarButtonX2(Ctl) do
...
procedure TToolbarButtonX2.SetDown (Value: Boolean);
begin
...
procedure TToolbarButtonX2.SetFlat (Value: Boolean);
begin
...
procedure TToolbarButtonX2.SetGroupIndex (Value: Integer);
begin
...
procedure TToolbarButtonX2.SetLayout (Value: TButtonLayout);
begin
...
procedure TToolbarButtonX2.SetMargin (Value: Integer);
begin
...
procedure TToolbarButtonX2.SetNoBorder (Value: Boolean);
begin
...
procedure TToolbarButtonX2.SetOldDisabledStyle (Value: Boolean);
begin
  {$IFDEF USEBUTTONGLYPH}
...
procedure TToolbarButtonX2.SetOpaque (Value: Boolean);
begin
...
procedure TToolbarButtonX2.Redraw (const Erase: Boolean);
var
  AddedOpaque: Boolean;
begin
...
procedure TToolbarButtonX2.SetSpacing (Value: Integer);
begin
...
procedure TToolbarButtonX2.SetAllowAllUp (Value: Boolean);
begin
...
procedure TToolbarButtonX2.SetDropdownMenu (Value: TPopupMenu);
var
...
procedure TToolbarButtonX2.SetWordWrap (Value: Boolean);
begin
...
procedure TToolbarButtonX2.SetAlignment (Value: TAlignment);
begin
...
procedure TToolbarButtonX2.SetDropdownAlways (Value: Boolean);
var
  NewUsesDropdown: Boolean;
begin
...
procedure TToolbarButtonX2.SetDropdownArrow (Value: Boolean);
begin
...
procedure TToolbarButtonX2.SetDropdownArrowWidth (Value: Integer);
var
  Diff: Integer;
begin
...
procedure TToolbarButtonX2.SetDropdownCombo (Value: Boolean);
var
  W: Integer;
begin
...
procedure TToolbarButtonX2.SetDisplayMode (Value: TButtonDisplayMode);
begin
...
{$IFDEF USEBUTTONGLYPH}
function TToolbarButtonX2.GetCallDormant: Boolean;
begin
  Result := TButtonGlyph(FGlyph).FCallDormant;
end;

procedure TToolbarButtonX2.SetCallDormant (Value: Boolean);
begin
...
{$IFDEF TB97D4}
function TToolbarButtonX2.IsCheckedStored: Boolean;
begin
  Result := (ActionLink = nil) or not TToolbarButtonX2ActionLink(ActionLink).IsCheckedLinked;
end;

function TToolbarButtonX2.IsHelpContextStored: Boolean;
begin
  Result := (ActionLink = nil) or not TToolbarButtonX2ActionLink(ActionLink).IsHelpContextLinked;
end;

function TToolbarButtonX2.IsImageIndexStored: Boolean;
begin
  Result := (ActionLink = nil) or not TToolbarButtonX2ActionLink(ActionLink).IsImageIndexLinked;
end;

procedure TToolbarButtonX2.ActionChange (Sender: TObject; CheckDefaults: Boolean);
begin
  inherited;
...
function TToolbarButtonX2.GetActionLinkClass: TControlActionLinkClass;
begin
  Result := TToolbarButtonX2ActionLink;
end;

procedure TToolbarButtonX2.AssignTo (Dest: TPersistent);
begin
  inherited;
...
procedure TToolbarButtonX2.WMLButtonDblClk (var Message: TWMLButtonDblClk);
begin
  inherited;
  if FDown then DblClick;
end;

procedure TToolbarButtonX2.CMEnabledChanged (var Message: TMessage);
begin
  if not Enabled then begin
    FState := bsDisabled;
...
procedure TToolbarButtonX2.CMDialogChar (var Message: TCMDialogChar);
begin
  with Message do
...
procedure TToolbarButtonX2.CMDialogKey (var Message: TCMDialogKey);
begin
  with Message do
...
procedure TToolbarButtonX2.CMFontChanged (var Message: TMessage);
begin
  Redraw (True);
end;

procedure TToolbarButtonX2.CMTextChanged (var Message: TMessage);
begin
  Redraw (True);
end;

procedure TToolbarButtonX2.CMSysColorChange (var Message: TMessage);
begin
  inherited;
...
procedure TToolbarButtonX2.MouseEntered;
begin
...
procedure TToolbarButtonX2.MouseLeft;
var
  OldState: TButtonStateX2;
begin
  if Enabled and FMouseInControl and not FMouseIsDown then begin
    ...
    if FFlat or ({$IFDEF USEBUTTONGLYPH}(NumGlyphs >= 5) or {$ENDIF} ((OldState = bsMouseIn) xor (FState <> OldState))) then
      Redraw (True);
...
procedure TToolbarButtonX2.ButtonMouseTimerHandler (Sender: TObject);
var
  P: TPoint;
begin
...
constructor TEditX2.Create (AOwner: TComponent);
begin
  inherited;
  AutoSize := False;
  {$IFNDEF FPC}
  ...
  Height := 19;
  if EditX2Count = 0 then
    RegisterX2ControlClass (TEditX2);
  Inc (EditX2Count);
end;

destructor TEditX2.Destroy;
begin
  Dec (EditX2Count);
  if EditX2Count = 0 then
    UnregisterX2ControlClass (TEditX2);
  inherited;
end;

procedure TEditX2.CMMouseEnter (var Message: TMessage);
begin
  inherited;
  MouseInControl := True;
...
procedure TEditX2.CMMouseLeave (var Message: TMessage);
begin
  inherited;
...
procedure TEditX2.NewAdjustHeight;
var
  DC: HDC;
  SaveFont: HFONT;
  Metrics: TTextMetric;
...
procedure TEditX2.CMEnabledChanged (var Message: TMessage);
const
...
procedure TEditX2.CMFontChanged (var Message: TMessage);
begin
  inherited;
...
procedure TEditX2.WMSetFocus (var Message: TWMSetFocus);
begin
  inherited;
  if not(csDesigning in ComponentState) then
    DrawNCArea (False, 0, 0);
...
procedure TEditX2.WMKillFocus (var Message: TWMKillFocus);
begin
  inherited;
  if not(csDesigning in ComponentState) then
    DrawNCArea (False, 0, 0);
...
procedure TEditX2.WMNCCalcSize (var Message: TWMNCCalcSize);
begin
  InflateRect (Message.CalcSize_Params^.rgrc[0], -3, -3);
end;

procedure TEditX2.WMNCPaint (var Message: TMessage);
begin
  DrawNCArea (False, 0, HRGN(Message.WParam));
end;

procedure TEditX2.DrawNCArea (const DrawToDC: Boolean; const ADC: HDC;
  const Clip: HRGN);
var
  DC: HDC;
  R: TRect;
  ...
  try
    ...
    if ((csDesigning in ComponentState) and Enabled) or
       (not(csDesigning in ComponentState) and
        (Focused or (MouseInControl and not ControlIsX2Control(Screen.ActiveControl)))) then begin
...
begin
  TEditX2(AppData).DrawNCArea (True, DC, 0);
end;
...
function TToolbarButtonX2.GetAvailableImageList: TCustomImageList;
//return self images, or TToolbar(parent).image
begin
  if Assigned(self.Images) then
    Result := self.Images
  else
  begin
    if Parent is TCustomToolWindowX2 then
    begin
      Result := TCustomToolWindowX2Access(Parent).Images;
    end;  
  end;  
end;
